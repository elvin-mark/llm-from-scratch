...
def test_numpy_vs_pytorch_forward_logits():
    ...
    model = TinyLLM(vocab_size=vocab_size, dim=dim, n_layers=n_layers, n_heads=n_heads, ffn_dim=ffn_dim)
    model.eval()
    ...
    assert np.allclose(pytorch_logits, numpy_logits, atol=1e-4), "NumPy forward logits do not match PyTorch model logits."
