/*
 * Native Apple Silicon Metal Shader Autoregressive Engine (c/run_metal.m).
 *
 * Runs 1.58-bit BitNetLLM autoregressive text generation directly on Apple M1/M2/M3/M4 GPUs
 * using Metal Compute Shaders and Unified Memory Architecture (MTLResourceStorageModeShared).
 *
...
int main(int argc, const char* argv[]) {
    @autoreleasepool {
        ...
        id<MTLFunction> swigluFunc = [library newFunctionWithName:@"swiglu_kernel"];
        ...
        id<MTLComputePipelineState> swigluPipeline = [device newComputePipelineStateWithFunction:swigluFunc error:&error];

        id<MTLCommandQueue> commandQueue = [device newCommandQueue];

        // Allocate Apple Silicon Unified Memory Buffers
        ...
        state.d_xb2 = [device newBufferWithLength:config.dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_hb = [device newBufferWithLength:config.ffn_dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_hb2 = [device newBufferWithLength:config.ffn_dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_q = [device newBufferWithLength:config.dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_k = [device newBufferWithLength:config.dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_v = [device newBufferWithLength:config.dim * sizeof(float) options:MTLResourceStorageModeShared];
        state.d_att = [device newBufferWithLength:config.n_heads * config.max_seq_len * sizeof(float) options:MTLResourceStorageModeShared];
        ...
        for (int pos = 0; pos < steps; pos++) {
            ...
            for (int l = 0; l < config.n_layers; l++) {
                // Metal Compute Command Dispatch
                ...
                // 2. 1.58-Bit Ternary BitLinear Q Projection (0 FP Multiplications!)
