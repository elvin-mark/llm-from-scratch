...
def train_lora():
    ...
    if not os.path.exists(corpus_path) or not os.path.exists(tokenizer_path):
        print("❌ Dataset or Tokenizer not found. Please run scripts/prepare_data.py first.")
        return
    ...
    # Load base model
    model = TinyLLM(vocab_size=vocab_size, dim=128, n_layers=4, n_heads=4, ffn_dim=512, max_seq_len=64)
    ...
    print(f"✅ Saved LoRA adapter weights ({len(adapter_state)} tensors) to checkpoints/lora_adapters.pth")
